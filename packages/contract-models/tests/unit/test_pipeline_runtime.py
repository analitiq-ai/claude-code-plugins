...
class TestBatchingHasNoConcurrencyKnob:
    ...
    bounded nothing. Be precise about the claim: the pipeline-level key WAS
    parsed downstream, it just never limited concurrency, which makes it dead
    config rather than ignored config. A knob that looks load-bearing and is
    inert is worse than an absent
