...
The document is GENERATED by `scripts/render_schemas.py` from the vendored
engine grammar (`analitiq.contracts.arrow_grammar`) and committed only by the
schema release, so these tests read the render, which the next release
publishes.
...
from render_schemas import build_arrow_types_doc


@pytest.fixture(scope="module")
def canonical_doc() -> dict:
    return build_arrow_types_doc()
