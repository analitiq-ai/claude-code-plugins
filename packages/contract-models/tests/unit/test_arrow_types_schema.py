...
test_endpoint_model.py / test_stream_arrow_types.py.

The document is GENERATED by `scripts/render_schemas.py arrow-types` from
the vendored engine grammar (`analitiq.contracts.arrow_grammar`);
...
covers the standalone arrow-types schema, published on its own at
`https://schemas.<domain>/arrow-types.json`, and pins its relaxed vocabulary
...
# arrow-types.json is generated but flat: it is not a registered renderer
...
ARROW_TYPES_PATH = REPO_ROOT / "schemas" / "arrow-types.json"


@pytest.fixture(scope="module")
def canonical_doc() -> dict:
    return json.loads(ARROW_TYPES_PATH.read_text())


@pytest.fixture(scope="module")
def registry(canonical_doc):
    """Registry so arrow-types.json `$ref`s resolve.

    The bare-filename binding (`arrow-types.json`) is required for the
    inline `$ref` strings used inside this test file. The absolute-URL form
    (`https://schemas.analitiq.ai/arrow-types.json`) also resolves — but via
    ...
    return Registry().with_resource("arrow-types.json", resource)
...
class TestCanonicalType:
    ...
    @pytest.mark.parametrize("value", [
        # Bare parameterized — a family whose params are mandatory, spelled
        # without them
        "Timestamp", "Decimal128", "Decimal256",
        "Time32", "Time64", "FixedSizeBinary",
        "Duration",
        # Families outside the executable vocabulary: typed nested
        ...
        "Decimal256(76, -5)",
        "Decimal128(38, 39)",    # scale above the family's precision ceiling
        "Decimal256(76, 77)",
...
# (`${1bad}`, `${ }`) are deliberately excluded: `_validate_type_map_arrow_type`
...
_PARITY_CORPUS = [
    ...
    "Decimal128(38, -2)", "Decimal128(1, 45)",  # scale above the family ceiling
    ...
    "Decimal128(${p}, 20)",  # satisfiable partial template
    ...
    "Decimal128(${p}, 99)",  # unsatisfiable: literal scale above precision's ceiling
]
...
def test_relaxed_vocabulary_matches_runtime(value, canonical_type_or_template_validator):
    ...
    substitute-then-fullmatch EXACTLY, so — besides the malformed placeholder
    NAMES excluded per the corpus note above — two exotic classes intentionally
    sit outside this corpus, with the runtime staying authoritative:
