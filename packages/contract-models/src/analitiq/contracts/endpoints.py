...
class _RequestBase(HeaderMergeRules, DeclaredHeaderNames, _EndpointModel):
    ...
    transport_ref: str | None = Field(
        ...
        description=(
            ...
            "has two halves, guaranteed by different mechanisms. "
            ...
            "(2) ORIGIN: every URL this request produces, including a "
            "next-page link followed from `pagination.link.next_url`, lands on "
            "the origin of the transport actually selected for the operation. "
            "This half is enforced by the engine at run time, on both the read "
            "and the write path, resolved per operation rather than against a "
            "single fixed transport — it is not checked by this contract or by "
            "`analitiq-validator`, both of which stop at the NAME half above."
        ),
    )
    path: str = Field(
        ...,
...
def _validate_arrow_type_in_json_schema(
    schema: Any, path: str, errors: list[RuleViolation], rule_id: str,
) -> None:
    ...
    for tokens, node in walk_structural_positions(schema):
        ...
        if has_native ^ has_arrow:
            ...
            errors.append(violation(
                rule_id, "native-arrow-pairing-incomplete",
        ...
        if has_arrow and isinstance(arrow_value, str):
            ...
            if arrow_value == "Object":
                ...
                if has_properties or has_items:
                    errors.append(violation(
                        rule_id, "json-container-shape-invalid",
                ...
                if has_properties or has_items:
                    errors.append(violation(
                        rule_id, "scalar-container-shape-invalid",
...
def _validate_schema_refs(schema: Any, path: str, errors: list[RuleViolation]) -> None:
    ...
    * the reference keywords in :data:`_REFUSED_REFERENCE_KEYWORDS`, refused
    ...
    Unlike :func:`_validate_arrow_type_in_json_schema`, no `rule_id` is
    threaded in from the caller: every complaint here is tagged by WHICH one
    fired, not by which model called this walker, since both RULE-ENDP-026
    and RULE-ENDP-064 already bind both `ResponseExtraction` and `WriteInput`.
    """
    for tokens, node in walk_structural_positions(schema):
        ...
        if "$ref" in node:
            ...
            if not isinstance(ref, str):
                ...
                errors.append(violation(
                    "RULE-ENDP-026", "ref-not-in-document",
                ...
                # is not there.
                errors.append(violation(
                    "RULE-ENDP-026", "ref-anchor-fragment",
                ...
                errors.append(violation(
                    "RULE-ENDP-026", "ref-resolves-to-boolean",
                ...
                if resolve_local_pointer(schema, ref) is not _MISSING:
                    errors.append(violation(
                        "RULE-ENDP-026", "ref-non-schema-position",
...
class ResponseExtraction(_EndpointModel):
    ...
    def _validate(self) -> "ResponseExtraction":
        ...
        _validate_arrow_type_in_json_schema(self.schema_, "response.schema", errors, "RULE-ENDP-005")
        _validate_schema_refs(self.schema_, "response.schema", errors)
        if errors:
            raise MultiRuleViolation(errors)
        return self
...
class WriteInput(_EndpointModel):
    ...
    def _validate(self) -> "WriteInput":
        errors: list[RuleViolation] = []
        _validate_arrow_type_in_json_schema(self.schema_, "input.schema", errors, "RULE-ENDP-006")
...
class _RecordsArray(NamedTuple):
    ...
    engine_record_properties: dict[str, Any] | None
    composed: dict[str, Any]


def _record_properties_as_the_engine_reads_them(
    ...
    items = node.get("items") if isinstance(node, dict) and node.get("type") == "array" else node
...
def _validate_records_in_response_schema(
    ...
    return _RecordsArray(
        engine_record_properties=_record_properties_as_the_engine_reads_them(
            response.schema_, segments
        ),
        composed=node,
    )
...
def _validate_cursor_fields_in_record_shape(
    ...
    what the engine's cursor reader finds: the whole name, looked up in the
...
def _check_cursor_field_holds_the_mapping(
    ...
    record shape or anywhere on the way to it — which the lookup does not
    merge.
    ...
    if field is None:
        raise violation(
            ...
            "shape's own `properties`, carrying its own `type` and any "
            "`format` it needs, because a branch alongside is not read for "
            "this field"
        )
...
def _cursor_node_by_declared_path(
    ...
    *,
    ...
    Answers only "is this path declared" — a question about the document.
    What the engine's cursor reader will find is a different question with a
    different answer, asked of
    :func:`_record_properties_as_the_engine_reads_them`.
    """
    try:
