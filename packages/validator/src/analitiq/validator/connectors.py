...
def _read_from(name: str, findings: list[dict]) -> list[dict]:
    ...
    `connector.json`, which carries no such node.
    """
...
def collect_type_maps(parent: Path | Location, *, rule: str | None) -> TypeMapSiblings:
    ...
    try:
        ...
        legacy_present = _legacy_type_map_present(parent)
    ...
    for path in siblings:
        ...
        if declared is None:
            findings.extend(
                (name, f) for f in _read_from(name, _type_map_discriminator_findings(doc)))
            continue
