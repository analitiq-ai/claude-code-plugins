...
validity to the contract models and adds the connector package's checks.
...
# (corpus file, its kind, expected pass?) — single-document verdicts.
DOC_CASES = [
...
@pytest.mark.parametrize("kind", ["connector", "api-endpoint", "database-endpoint", "type-map"])
def test_a_document_of_no_kinds_shape_errors(validator, kind):
    # Whatever kind the caller names, a document not in its shape fails
    # rather than passing on the strength of nothing having matched.
...
def _api_package(connector: dict, read_map, endpoints: dict) -> dict:
    """A connector package: `connector`, a type map carrying `read_map`, and
    each endpoint under its file name."""
    return {_CONNECTOR: connector, _TYPE_MAP: _type_map_doc(read=read_map),
            **{f"{_ENDPOINTS}/{name}": ep for name, ep in endpoints.items()}}


def _graded(validator, documents: dict) -> list:
    """The findings for `documents` graded as a connector package, where a
    string value is a document's text as written."""
    texts = {key: doc if isinstance(doc, str) else json.dumps(doc)
             for key, doc in documents.items()}
    request = ValidatePackageRequest(package="connector-package", documents=texts)
...
def test_directional_pairs_the_read_map_can_render(connector_base, validator, rules, read_pair, write_pair):
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": n, "arrow_type": c} for n, c in rules],
                {"widgets.json": _read_and_write(*read_pair, *write_pair)})
    errors = _errors(_graded(validator, documents))
...
def test_coverage_still_runs_when_the_read_map_fails_its_model(connector_base, validator):
    ...
    # about it the model rejects. The package check reads that
    ...
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
                {"widgets.json": _endpoint("BIGINT", "Int64")})
    documents[_TYPE_MAP]["$schema"] = CONNECTOR_SCHEMA_URL
    errors = _errors(_graded(validator, documents))
    assert any(e["path"] == f"{_TYPE_MAP}#/$schema" for e in errors), errors
...
def test_coverage_flags_missing_read_map(connector_base, validator):
    errors = _errors(_graded(validator, {_CONNECTOR: connector_base}))
    assert any(e["message_id"] == "read-map-missing" for e in errors), errors


def test_an_endpoint_that_holds_a_json_null_is_graded(connector_base, validator):
    ...
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
                {"widgets.json": _endpoint("STRING", "Utf8")})
    documents[f"{_ENDPOINTS}/widgets.json"] = "null"
    errors = _errors(_graded(validator, documents))
...
def test_coverage_json_narrowing_allowed(connector_base, validator):
    ...
    assert not _errors(_graded(validator, documents))
...
def test_coverage_json_narrowing_is_narrow(connector_base, validator):
    ...
    assert _errors(_graded(validator, documents))
...
def test_read_coverage_returns_promptly_on_a_nested_quantifier(connector_base, validator):
    ...
    findings = _within(5, lambda: _graded(validator, documents))
...
def test_coverage_flags_duplicate_endpoint_id(connector_base, validator):
...
def test_coverage_non_dict_endpoint_file_no_crash(connector_base, validator):
    ...
    documents = _api_package(connector_base,
                             [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
                             {"widgets.json": []})
    errs = _errors(_graded(validator, documents))
    assert errs
...
def test_coverage_flags_keyset_explicit_null_initial(connector_base, validator):
    # A package grades each endpoint it locates as an api-endpoint, so a defect
    # the endpoint settles alone reaches the package's verdict too.
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
...
# --- coverage matrix (the package check alone isolates it from model validity) ---
...
def _mapped(kind: str, sections) -> dict:
    """A `kind` connector's package holding a type map carrying a valid rule
    list for each direction in `sections`, or no map at all when `sections`
    is None."""
    documents = {_CONNECTOR: _min_connector(kind)}
    if sections is not None:
        documents[_TYPE_MAP] = _type_map_doc(**{d: _SECTION_RULES[d]() for d in sections})
    return documents
...
def _at_map(findings: list) -> list:
    return [f for f in findings if f["path"].startswith(f"{_TYPE_MAP}#")]
...
def test_a_kind_is_held_to_the_sections_its_map_carries(kind, sections, expected):
    ...
    findings = _checked(_mapped(kind, sections))
...
def test_a_null_section_is_an_absent_one(kind, expected):
    ...
    findings = _checked({_CONNECTOR: _min_connector(kind),
                         _TYPE_MAP: {"$schema": _TM_SCHEMA, "read": _read_rules(), "write": None}})
    assert {f["message_id"] for f in findings if f.get("rule") == "RULE-PKG-030"} == expected, findings
...
def test_a_malformed_section_is_not_a_missing_one(validator, section):
    ...
    findings = _graded(validator, {_CONNECTOR: _min_connector("database"), _TYPE_MAP: type_map})
    assert f"{_TYPE_MAP}#/{section}" in {f["path"] for f in _errors(findings)}, findings
    assert not {f["message_id"] for f in findings if f.get("rule") == "RULE-PKG-030"}, findings


def test_a_refused_write_section_is_not_held_to_the_write_vocabulary():
    ...
    rules = {f.get("rule") for f in _checked(_mapped("api", ("read", "write")))}
    assert "RULE-PKG-030" in rules and "RULE-TMAP-017" not in rules, rules


@pytest.mark.parametrize("sections,reason", [
    (None, "the package holds no type map"),
    (("write",), "its type map carries no 'read' section"),
])
def test_a_missing_section_says_whether_the_package_holds_a_map(sections, reason):
    # A package without a map asks for a different edit than one whose map
    # lacks the section, so the message says which it is.
    [missing] = [f for f in _checked(_mapped("database", sections))
                 if f["message_id"] == "read-map-missing"]
...
def test_a_map_finding_names_the_file_it_was_read_from(validator, payload, expected_id):
    # A document finding locates itself with a pointer into the document it
    # graded. Forwarded beside a connector it reads as a defect in the
    # connector, which carries no such node, so the author is told what is
    # wrong and cannot tell which file to open.
    reported = _graded(validator, {_CONNECTOR: _min_connector("api"), _TYPE_MAP: payload})
    ...
    assert named[0]["path"].startswith(f"{_TYPE_MAP}#/"), named[0]


@pytest.mark.parametrize("kind", (*_DATABASE_KINDS, *_STORAGE_KINDS))
def test_coverage_holds_a_connector_write_map_to_the_whole_vocabulary(kind):
    ...
    findings = _checked(_mapped(kind, ("read", "write")))
    assert "RULE-TMAP-017" in {f.get("rule") for f in findings}, findings


def test_a_type_map_graded_alone_is_not_held_to_the_write_vocabulary(validator):
    # A type map on its own is graded as a connection map may be: gap-only.
    # What a connector's map must render is the connector package's to ask.
    findings = validator.validate_document(_type_map_doc(write=_write_rules()), "type-map")
    assert "RULE-TMAP-017" not in {f.get("rule") for f in findings}, findings


@pytest.mark.parametrize("kind", _STORAGE_KINDS)
def test_a_storage_map_is_graded_in_every_section_it_carries(validator, kind):
    ...
    errors = _errors(_graded(validator, {_CONNECTOR: _min_connector(kind), _TYPE_MAP: type_map}))
    assert [e["path"] for e in _at_map(errors)] == [f"{_TYPE_MAP}#/write/0/arrow_type"], errors
...
def test_a_coverage_finding_names_the_map_it_was_rendered_from(
    ...
    documents = _api_package(connector_base, _read_rules(),
                             {"widgets.json": _endpoint(native_type, arrow_type)})
    named = [e for e in _errors(_graded(validator, documents)) if e["message_id"] == message_id]
    assert named, documents
    assert "type map" in named[0]["message"], named[0]
...
@pytest.mark.parametrize("text", ["[ not json", *_PARSER_REFUSALS.values()],
                         ids=["malformed", *_PARSER_REFUSALS])
def test_a_map_the_parser_refuses_is_reported_as_unreadable_and_nothing_else(validator, text):
    # Nothing was read, so there is no document to grade: a model finding over
    # it would describe a value the file never held. Uncaught, a refusal
    # outside `JSONDecodeError` would end the whole package's grading instead.
    findings = _graded(validator, {_CONNECTOR: _min_connector("file"), _TYPE_MAP: text})
    assert [f["message_id"] for f in _at_map(findings)] == ["unreadable-document"], findings
...
def test_cli_reports_a_document_the_parser_refuses_as_unreadable(
    ...
    monkeypatch.setattr("sys.argv", ["validator", "--document", str(document), "--kind", "connector"])
    assert _core.main() == 1
...
def test_a_short_wrong_tag_comes_back_with_every_accepted_value(validator):
    ...
    tag = [e for e in _errors(validator.validate_document(doc, "connector"))
...
def test_a_borrowed_diagnostic_keeps_the_constraint_it_was_rejected_by(validator):
    ...
    [error] = _errors(validator.validate_document(doc, "type-map"))
...
def test_regex_lowercase_literal_warning_truth_table(validator, native, verdict, witness):
    ...
    findings = validator.validate_document(
        _type_map_doc(read=[{"match": "regex", "native_type": native, "arrow_type": "Utf8"}]), "type-map")
...
def test_regex_read_rule_container_warning(validator, native, arrow_type, warns):
    findings = validator.validate_document(
        _type_map_doc(read=[{"match": "regex", "native_type": native, "arrow_type": arrow_type}]), "type-map")
...
def test_regex_warnings_stay_cheap_across_a_whole_map(validator):
    ...
    findings = validator.validate_document(
        _type_map_doc(read=rules), "type-map")
    elapsed = time.perf_counter() - started
    assert sum(w.get("rule") == "RULE-TMAP-014" for w in _warnings(findings)) == 50, findings
    assert elapsed < 1.0, elapsed


def _connector_write_map_findings(write_rules) -> list:
    """What a database connector's package check reports about a type map
    carrying `write_rules`: the write vocabulary is held at connector scope."""
    return _checked({_CONNECTOR: _min_connector("database"), _TYPE_MAP: _type_map_doc(write=write_rules)})


def test_write_vocabulary_gap_warns():
    # A write map missing whole canonical families → advisory warning.
    findings = _connector_write_map_findings(
        [{"match": "exact", "arrow_type": "Utf8", "native_type": "TEXT"}])
    assert any(w.get("rule") == "RULE-TMAP-017" for w in _warnings(findings))


def test_write_vocabulary_probes_bare_container_markers():
    ...
    findings = _connector_write_map_findings(covered)
...
def test_write_vocabulary_fully_covered_map_warns_nothing():
    ...
    findings = _connector_write_map_findings(full_map)
...
_BROKEN_READ_MAPS = (
    ("missing", None, "the package holds no type map"),
...
_ENDPOINT_DEFECTS = {
    ...
    "RULE-ENDP-047": "is not declared in the connector's `transports`",
...
def _defective_endpoints(connector: dict, read_map_text: str | None) -> dict:
    """A connector package carrying every defect `_ENDPOINT_DEFECTS` names,
    beside a map in one of the broken states (or none at all)."""
    documents = {_CONNECTOR: connector}
    if read_map_text is not None:
        documents[_TYPE_MAP] = read_map_text
    ...
    documents[f"{_ENDPOINTS}/misnamed.json"] = ep
    ...
    documents[f"{_ENDPOINTS}/meta.json"] = meta
    return documents
...
def _assert_endpoint_checks_survive(connector, validator, read_map_text, reported):
    errors = _errors(_graded(validator, _defective_endpoints(connector, read_map_text)))
...
def test_unrendered_coverage_is_reported_not_silent(connector_base, validator, text):
    ...
    findings = _graded(validator, _defective_endpoints(connector_base, text))
...
def _with_an_endpoint(documents: dict) -> dict:
    """`documents` plus an api endpoint, which a database-family package has no
    business shipping."""
    endpoint = _endpoint("STRING", "Utf8", endpoint_id="widgets", path="/v1/widgets")
    return {**documents, f"{_ENDPOINTS}/misnamed.json": endpoint}


def test_database_missing_both_sections_reports_both():
    ...
    # branch stays terminal, so an endpoint beside it is neither enumerated nor
    # demanded.
    documents = _mapped("database", None)
    with_endpoint = _checked(_with_an_endpoint(documents))
    assert with_endpoint == _checked(documents), with_endpoint
    assert {"read-map-missing", "write-map-missing"} <= {
        e["message_id"] for e in _errors(with_endpoint)}, with_endpoint
    assert not [f for f in with_endpoint if "endpoint" in f["message"]], with_endpoint


_COVERAGE_RULE_IDS = {"RULE-PKG-030", "RULE-PKG-032", "RULE-PKG-033", "RULE-PKG-035"}


def test_clean_tree_emits_no_coverage_finding(connector_base, validator):
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
                {"widgets.json": _endpoint("STRING", "Utf8")})
    findings = _graded(validator, documents)
...
def test_database_family_never_enumerates_endpoints(kind):
    # A database connector's release ships no endpoint documents, so the
    # package check reads endpoints for api connectors only — a broken read
    # section does not change that. An endpoint in the package therefore moves
    # nothing in its verdict, and a rendering the kind never asks for is not
    # warned about either.
    documents = {_CONNECTOR: _min_connector(kind),
                 _TYPE_MAP: {"$schema": _TM_SCHEMA, "read": {}, "write": _write_rules()}}
    with_endpoint = _checked(_with_an_endpoint(documents))
    assert with_endpoint == _checked(documents), with_endpoint
    assert not [f for f in with_endpoint if "endpoint" in f["message"]], with_endpoint
    assert not [f for f in with_endpoint if "not rendered" in f["message"]], with_endpoint
...
def test_cli_valid_doc_exit0(validator_cli):
    ...
    r = validator_cli.on_document(doc, "api-endpoint")
...
def test_cli_invalid_doc_exit1(validator_cli):
    r = validator_cli.on_document(
        json.loads((CORPUS / "invalid_write_from_input.json").read_text()), "api-endpoint")
...
def test_cli_unreadable_document_exit1(tmp_path, validator_cli):
    ...
    r = validator_cli.run("--document", str(tmp_path), "--kind", "connector")
    assert r.returncode == 1
    assert json.loads(r.stdout)["passed"] is False
...
def test_cli_text_refused_outside_jsondecodeerror_is_unreadable(
    ...
    r = validator_cli.run("--document", str(path), "--kind", "connector")
...
# produced them: an api-endpoint graded alone and graded inside its package
# agree, and a connector-shaped dict without its `kind` still reports what a
# connector does.
...
def test_kindless_connector_still_reports_a_missing_schema_url(validator):
    ...
    doc = {"connector_id": "x", "transports": {}, "connection_contract": {},
           "default_transport": "m"}
    findings = validator.validate_document(doc, "connector")
    assert any(f.get("rule") == "RULE-SHRD-003" for f in findings), findings


def test_endpoint_findings_locate_the_same_node_on_both_routes(connector_base, validator):
    # RULE-ENDP-048 locates the offending schema. Graded alone, the endpoint is
    # the whole address, so a bare pointer locates it; graded in its package,
    # the pointer follows the endpoint's key. Resolved, the two name one node,
    # and the message is the same sentence on both.
    ep = _endpoint("STRING", "Utf8")
    ep["operations"]["read"]["response"]["schema"]["minItems"] = "notanumber"
    documents = _api_package(connector_base,
                [{"match": "exact", "native_type": "STRING", "arrow_type": "Utf8"}],
    ...
    [via_package] = [f for f in _graded(validator, documents)
                     if f.get("rule") == "RULE-ENDP-048"]
    [via_endpoint] = [f for f in validator.validate_document(ep, "api-endpoint")
                      if f.get("rule") == "RULE-ENDP-048"]

    assert via_package["path"] == f"{_ENDPOINTS}/widgets.json#/operations/read/response/schema"
    assert via_endpoint["path"] == "/operations/read/response/schema"
    assert via_package["message"] == via_endpoint["message"]


# ---------------------------------------------------------------------------
# One type-map document, graded as its kind.
# ---------------------------------------------------------------------------
...
def test_a_type_map_grades_each_section_as_its_direction(validator):
    ...
    assert not _errors(validator.validate_document(_type_map_doc(read=rule), "type-map"))
    errors = _errors(validator.validate_document(_type_map_doc(write=rule), "type-map"))
...
def test_a_type_map_points_each_advisory_into_its_section(validator):
    ...
    duplicates = [f["path"] for f in validator.validate_document(doc, "type-map")
...
def test_a_type_map_reads_no_section_as_the_other_direction(validator):
    ...
    assert validator.validate_document(doc, "type-map") == []
