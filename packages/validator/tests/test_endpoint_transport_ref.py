...
transport its package's connector declares.
...
where both documents are in hand. The tests here drive it through
`validate_package` over a connector package.
"""
import json
...
def _run(connector, endpoints, validator):
    """Findings for a connector package carrying `connector`, a read map
    covering the endpoints' native type, and each of `endpoints` under
    `definition/endpoints/<name>`; an endpoint given as text is carried as-is."""
    documents = {
        "definition/connector.json": json.dumps(connector),
    ...
    for name, ep in endpoints.items():
        documents[f"definition/endpoints/{name}"] = ep if isinstance(ep, str) else json.dumps(ep)
    return validator.validate_package(ValidatePackageRequest(
        package_kind="connector", documents=documents))["findings"]
...
def test_malformed_endpoint_does_not_crash_the_check(connector_base, ep, validator):
    """The package's endpoint checks run over documents that may already carry
    model errors; a crash here would replace every actionable finding with a
    generic "validator bug". Each malformed shape must still produce findings and no
...
class TestOriginContainmentIsAValidatorBlindSpot:
    ...
    validates clean here. They run through the connector package, which is
    ...
    Three documents, because they reach the package's checks by different legs
    and a check need not cover every one: a read request, a write request, and
