...
grading as a single endpoint document and as an endpoint inside a connector
package, whose findings name the endpoint's key.
"""
import json
import os
import subprocess
...
from conftest import SUBPROCESS_DEADLINE_SECONDS
...
def _endpoint_request(doc):
    return ValidateSingleDocumentRequest(document_kind="api-endpoint", document=json.dumps(doc))


def _endpoint_findings(validator, doc):
    return validator.validate_single_document(_endpoint_request(doc))["findings"]


def _package_request(endpoint):
    """A connector package carrying the corpus connector, a read map covering
    `STRING_FLAG`'s native type, and `endpoint` under its id's key."""
    corpus = Path(__file__).resolve().parent / "corpus" / CORPUS_CONNECTOR
    return ValidatePackageRequest(package_kind="connector", documents={
        "definition/connector.json": corpus.read_text(),
        "definition/type-map.json": json.dumps(
            {"$schema": TYPE_MAP_SCHEMA_URL,
             "read": [{"match": "exact", "native_type": "BOOLEAN", "arrow_type": "Boolean"}]}),
        f"definition/endpoints/{endpoint['endpoint_id']}.json": json.dumps(endpoint),
    })


def _model_errors(doc):
    """Whether the api-endpoint contract model itself rejects `doc` — checked
    directly against the model rather than by filtering the validator's
...
def test_string_flag_under_a_boolean_node_errors(validator):
    ...
    findings = _endpoint_findings(validator, doc)
...
def test_a_write_input_node_is_graded_in_every_mode(mode, validator):
    ...
    errors = _sample_findings(_endpoint_findings(validator, doc))
...
def test_a_node_reached_through_defs_and_composition_is_graded(validator):
    ...
    paths = {e["path"] for e in _sample_findings(_endpoint_findings(validator, doc))}
    assert paths == {
...
def test_a_node_asserting_nothing_is_graded_against_nothing(validator):
    ...
    assert not _sample_findings(_endpoint_findings(validator, doc))
...
def test_no_authored_string_defeats_the_finding_size_bound(node, validator):
    ...
    errors = _sample_findings(_endpoint_findings(validator, _read_endpoint({"a": node})))
    assert len(errors) == 1, errors
...
def test_a_package_locates_findings_under_the_endpoint_key(validator):
    """In a connector package a finding must name the endpoint document it came
    from."""
    result = validator.validate_package(_package_request(_read_endpoint({"paid": STRING_FLAG})))
    errors = _sample_findings(result["findings"])
    assert len(errors) == 2, result
...
#: Runs the entry point named in argv over the request on stdin and writes the
#: envelope to stdout.
_ENTRY_POINT_PROGRAM = """\
import json, sys
from analitiq.contracts import validation_requests
from analitiq.validator import document_set
entry_point, request_model = sys.argv[1:]
request = getattr(validation_requests, request_model).model_validate_json(sys.stdin.read())
json.dump(getattr(document_set, entry_point)(request), sys.stdout)
"""


def _validated_in_child(entry_point, request):
    """`entry_point` over `request`, run in a child process.

    A regression in the bound does not make this check answer wrongly, it makes it
    not answer, so a direct call would hang the suite where the deadline fails it.
    """
    try:
        proc = subprocess.run(
            [sys.executable, "-c", _ENTRY_POINT_PROGRAM, entry_point, type(request).__name__],
            input=request.model_dump_json(), capture_output=True, text=True,
            timeout=SUBPROCESS_DEADLINE_SECONDS,
            env={**os.environ, "PYTHONPATH": os.pathsep.join(p for p in sys.path if p)},
            check=False)
    except subprocess.TimeoutExpired:
        pytest.fail(f"{entry_point} did not return within {SUBPROCESS_DEADLINE_SECONDS}s")
    assert proc.returncode == 0, proc.stderr
    return proc


def _sample_findings_in_child(doc):
    """This endpoint's RULE-ENDP-063 findings, graded in a child process."""
    proc = _validated_in_child("validate_single_document", _endpoint_request(doc))
    return _sample_findings(json.loads(proc.stdout)["findings"])
...
def test_a_sample_past_the_budget_is_reported_and_costs_only_itself():
    ...
    doc = _read_endpoint({"before": {"type": "integer", "examples": ["one"]},
                          ...
                          "paid": {"type": "boolean", "examples": ["0"]}})
    errors = _sample_findings_in_child(doc)
    by_field = {e["path"].split("/properties/")[1]: e["message"] for e in errors}
...
def test_the_budget_bounds_every_keyword_not_only_pattern(node):
    ...
    errors = _sample_findings_in_child(_read_endpoint({"bag": node}))
    assert len(errors) == 1, errors
    assert "was not graded" in errors[0]["message"]


def test_a_package_bounds_a_pathological_sample():
    """A package reaches the same grading through a different caller, so a bound
    applied at the single-document call site would leave a connector package
    unprotected."""
    proc = _validated_in_child(
        "validate_package", _package_request(_read_endpoint({"code": dict(_RUNAWAY_NODE)})))
    errors = _sample_findings(json.loads(proc.stdout)["findings"])
    assert len(errors) == 1, errors
    assert errors[0]["path"].startswith("definition/endpoints/widgets.json#/operations/read/"), errors
    assert "was not graded" in errors[0]["message"]
...
def test_a_document_that_is_not_json_reports_every_sample_rather_than_crashing():
    ...
    from analitiq.validator._core import validate_document_as

    doc = _read_endpoint({"amount": {"type": "integer", "examples": [Decimal("1.5")]},
                          "paid": {"type": "boolean", "examples": ["0"]}})
    findings = validate_document_as("api-endpoint", doc)
...
def test_a_budget_breach_leaves_no_traceback_on_stderr():
    ...
    raise, and the interpreter prints the traceback on the stderr of the process
    running the validator — which is exactly where a caller looks when the report
    is missing."""
    proc = _validated_in_child(
        "validate_single_document", _endpoint_request(_read_endpoint({"code": dict(_RUNAWAY_NODE)})))
...
def test_a_value_json_would_normalise_is_reported_rather_than_converted(node, sample):
    ...
    errors = _sample_findings(validate_document_as("api-endpoint", doc))
    assert len(errors) == 1, errors
    assert "was not graded" in errors[0]["message"]
...
def test_a_worker_ends_itself_when_its_deadline_passes():
    ...
    proc = subprocess.run(
        ...
        capture_output=True, text=True, timeout=SUBPROCESS_DEADLINE_SECONDS,
        env={**os.environ, "PYTHONPATH": os.pathsep.join(p for p in sys.path if p)},
        check=False)
