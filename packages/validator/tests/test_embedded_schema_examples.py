...
def _sample_findings_via_cli(validator_cli, doc, kind="api-endpoint", filename="doc.json"):
    ...
    """
    result = validator_cli.on_document(doc, kind, filename)
...
def test_the_connector_walk_bounds_a_pathological_sample(tmp_path, validator_cli):
    ...
    errors = _sample_findings_via_cli(validator_cli, connector, "connector", "connector.json")
    assert len(errors) == 1, errors
