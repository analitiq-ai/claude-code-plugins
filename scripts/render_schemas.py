...
def build_contracts_version_doc() -> dict[str, Any]:
    return {
        "$comment": (
            ...
            "analitiq-contract-models release this schema tree renders from. "
            "Do not hand-edit; run `render_schemas.py contracts-version` "
...
def check_contracts_version() -> tuple[bool, str]:
    ...
    if CONTRACTS_VERSION_PATH.read_text() != rendered:
        return (
            ...
            "(the stamp must equal the contract-models [project] version); "
            f"re-run {hint}",
        )
    return (
        True,
