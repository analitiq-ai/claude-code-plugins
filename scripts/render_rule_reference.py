...
#: The artifact kind that binds every authored document. It is not a bucket:
#: rules binding `any` are appended to EVERY file in the set, because the
#: premise of splitting by artifact kind is that one file is the whole of
#: what its reader must satisfy. Every `any`-bound rule duplicated into every
#: file is the price of that premise, and the rows are generated, so the
#: duplication is a rendering, not a second source.
ANY_ARTIFACT_KIND = "any"
...
def buckets(owner: str) -> dict[str, list]:
    ...
    universal = sorted(
        (r for r in rules if ANY_ARTIFACT_KIND in r.artifact_kinds), key=lambda r: r.id
    )

    by_artifact_kind: dict[str, list] = {}
    for rule in rules:
        for kind in rule.artifact_kinds:
            if kind != ANY_ARTIFACT_KIND:
                by_artifact_kind.setdefault(kind, []).append(rule)

    out: dict[str, list] = {}
    shared: list = []
    for kind, owned in sorted(by_artifact_kind.items()):
        (out.setdefault(kind, []) if len(owned) >= BUCKET_FLOOR else shared).extend(owned)
    if shared:
...
def render(bucket: str, rules: list, models: dict | None = None) -> str:
    ...
    grades_line = (
        SHARED_LINE
        if bucket == SHARED_BUCKET
        else ARTIFACT_KIND_LINE.format(bucket=bucket, article=_article(bucket))
    )
...
def render_all(owner: str) -> dict[Path, str]:
    ...
    # Coverage, asserted rather than assumed. With plural artifact kinds a
    # rule can land in several files, so "exactly once" is no longer the invariant —
...
def main(argv: list[str]) -> int:
    ...
    for owner in owners:
        ...
        if mode == "write":
            ...
            # A bucket that stops existing (its rule count fell under the floor, or
