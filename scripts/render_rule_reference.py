...
#: `resource_discovery` produces one, so `RULE-DBEP-*` binds an author there.
...
#: satisfy. Thirteen duplicated rows per file is the price of that premise, and
#: they are generated, so the duplication is a rendering, not a second source.
ANY_SCOPE = "any"
...
SHARED_LINE = (
    "Scope: the rules this plugin owns whose artifact kinds are too few to "
    "carry their own file, plus the rules that bind every authored document. "
    "Read it alongside the file for the document you are authoring, never "
    "instead of it."
)
...
def main(argv: list[str]) -> int:
    ...
    if mode not in ("write", "check"):
        print(f"usage: {argv[0]} [write|check] [--plugin <owner>]", file=sys.stderr)
        return 2

    owners = list(OUTPUT_DIRS)
    for flag in flags:
        if flag.startswith("--plugin="):
            owners = [flag.split("=", 1)[1]]
    if "--plugin" in flags:
        owners = [args[args.index("--plugin") + 1]] if "--plugin" in args else owners
