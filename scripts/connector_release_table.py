...
CATEGORIES: tuple[Category, ...] = (
    ...
    Category(
        ...
        "Filter operators narrowed",
        note=(
            "a release migrating from a param's `operators` to a `filters` map "
            "is this when the key a stream writes moves with it — the filter "
            "field named the param and now names a record field, so every "
            "stream naming the old key stops resolving. A migration where the "
            "two coincide and the request is unchanged moved nothing a stream "
            "wrote or sent and is not this. Otherwise: an operator a read "
            "operation offered on a "
            "field under `filters` "
    ...
    Category(
        ...
        note=(
            "a field and operator both releases offer would build a different "
            "request for the same filter value, so the same stream filter may "
            "read different rows. Any difference counts and the test is the "
            "request, never a list of attributes: a body binding moved to "
            "another position, a container serialized as a joined value where "
            "it was repeated keys, a different slot or wire key. Judged "
    ...
    Category(
        ...
        note=(
            ...
            "block removed so a stream silently reads one page, a param a "
            "filter binds whose request-value contract tightened anywhere — a bound, "
