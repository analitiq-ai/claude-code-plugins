...
| `endpoint-filename` | For api-endpoint documents: the file's basename must equal `{endpoint_id}.json` (the engine locates an endpoint as `endpoints/{endpoint_id}.json`, so a divergent filename is unreachable at runtime). The JSON Schema layer constrains `endpoint_id` but cannot see the filename, so this is semantic-only. An error when they diverge; a **warning** when the validator was invoked without a filesystem-anchored document path (basename unavailable). Enforced on every sibling endpoint during connector validation and on an endpoint validated directly. |
