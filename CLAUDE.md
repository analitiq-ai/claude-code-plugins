...
- `connector-schema-validator` — validates a document against the pinned contract models (structure plus every `ADV-*` cross-field rule, all reported under `contract-model`) and adds the cross-file checks a single document can't express (`type-map-coverage`, `type-map-rule`, `type-map-write-coverage`, `endpoint-filename`, `endpoint-id-unique`, `endpoint-id-locator`, `embedded-json-schema`). JSON documents only — package files are registry CI's job. Backed by the published **`analitiq-validator`** package (offline, model-driven), which the agent self-installs at runtime; registry CI installs the same package to run the checks standalone. The id list is pinned by `tests/schema_drift/`.
...
- **Connector:** Reusable provider transport + auth contract. Lives in `{connector_id}/definition/connector.json` and validates against `https://schemas.analitiq.ai/connector/latest.json`. Top-level fields: `$schema`, `kind` (one of `api`, `database`, `nosql`, `document`, `file`, `s3`, `stdout`), `connector_id` (the stable slug, matching `^[a-z0-9][a-z0-9_-]*$`; the on-disk directory uses the same value), `version`, `default_transport`, `transports`, `auth`, `connection_contract`, optional `resource_discovery`. Registry-stamped fields (`created_at`, `updated_at`) must NOT appear in authored documents.
...
- **Type maps:** Two **standalone** sibling files sharing the `{match, native, canonical}` rule shape (top-level **JSON array** — never an object wrapper; **first-match-wins**; `match` is a closed enum `exact` | `regex`). Each direction has its own published schema: the read map validates against `https://schemas.analitiq.ai/type-map-read/latest.json` and the write map against `https://schemas.analitiq.ai/type-map-write/latest.json`, both running the full contract-model + semantic pass (the validator derives the direction from the filename). The pre-split `type-map.json` filename is rejected with a migration finding.
  - **Read map** (`{connector_id}/definition/type-map-read.json`, native → Arrow): required and non-empty for both API and DB. `native` is the matcher; **author every matcher uppercase, with whitespace as documented** — the validator uppercases only the incoming native and compares the matcher verbatim (the runtime also normalizes an `exact` rule's native, so it is more forgiving there; `regex` patterns are verbatim in both — author for the stricter one). Coverage probes natives on API connectors only, so a lowercase `exact` native errors there and ships unchecked on a database connector (a lowercase `regex` literal warns on either kind). Capture-group names stay lowercase. `canonical` is the render side; regex rules may template it with `${name}` substitutions **backed by ECMA-262 `(?<name>…)` named captures in `native`** (anonymous groups don't count). Python-only `(?P…)` syntax is rejected. Schemaless / structured-container natives (`JSON`, `JSONB`, `VARIANT`, `OBJECT`, `ARRAY`, `MAP`, `array<…>`, `…[]`, …) must resolve to a **container canonical** (`Json`, or a typed `List`/`Struct`/`Map`), never a scalar like `Utf8`. Only the syntactic half is enforced (a visibly-container shape — `<…>` or a `[]` suffix — via ADV-TMAP-001/002); a bare vendor spelling like `JSONB → Utf8` validates clean, so it is on the author. See `src/skills/connector-spec-db/spec-type-maps.md`. `Object` / `List` are endpoint-only markers accepted as narrowings of a `Json`-resolved rule. Per-connector native→canonical specifics live in `src/skills/connector-spec-db/spec-type-maps.md`, not here.
  - **Write map** (`{connector_id}/definition/type-map-write.json`, Arrow → native DDL): **required for `kind: database`, forbidden for `kind: api`.** Direction inverts: `canonical` is the matcher (regex with named captures for parameterized types, matched verbatim — PascalCase is case-significant) and `native` is the rendered DDL (`${name}` substitutions backed by captures in `canonical`). Must cover the full canonical vocabulary; `type-map-write-coverage` warns on unrendered families but probes only a sample, so a clean run is a floor rather than proof (the families to check by hand are listed once in `src/skills/connector-spec-db/spec-type-maps.md` — don't restate them). A gap is legitimate only when the dialect overrides `render_column_type` for that family (BigQuery's Decimal). Connectors never ship Python type-rendering tables.
...
- **Value expression:** One of `ref` / `template` / `literal` / `function`. Refs and template variables lead with one of the contract's resolution scopes (`RESOLUTION_SCOPES`, pinned by `tests/schema_drift/`); the authoring guide to them is `src/skills/connector-builder/references/value-expressions.md`. Inline functions: `basic_auth`, `base64_encode`, `lookup`, `url_encode` (`jwt_sign` / `pkce_challenge_s256` are planned, not yet registered). Scope checking is narrow and never proves resolvability (the exact surface is in `value-expressions.md`); unknown **function names are never validated anywhere**. Both classes of mistake fail at connect, not at authoring time.
...
- **Driver selection (database):** decision order — (1) first-class ADBC driver (schema `AdbcTransport.driver` enum is the sole validator: `postgresql`, `snowflake`, `bigquery`; Redshift takes ADBC `postgresql`) → (2) Arrow Flight SQL endpoint → (3) async SQLAlchemy + native bulk path in the connector class (thick) → (4) async SQLAlchemy batched INSERT (last resort). Never the JDBC bridge. SQLAlchemy drivers must be async (`postgresql+asyncpg`, `mysql+aiomysql`, `mariadb+aiomysql`); the driver ships ONLY in the connector's `requirements.txt` (the engine pins no drivers; it derives the dbapi module as `adbc_driver_{driver}.dbapi`). Guide: `src/skills/connector-spec-db/spec-driver-selection.md`.
- **Connector package (database):** the connector root IS an installable Python package — `connector.py` (`{Name}Dialect(SqlDialect)` + `{Name}Connector(GenericSQLConnector)`; CDK imports only, never another connector or the engine), `__init__.py`, `requirements.txt` (this connector's drivers only), `pyproject.toml` (`analitiq-connector-{connector_id}`, dynamic deps from requirements.txt, entry points named `{connector_id}` under both `analitiq.source_connectors` and `analitiq.destination_connectors`). Dialects implement the hooks their transports require (`build_tls_connect_arg`, `build_sqlalchemy_upsert` + `supports_upsert_sqlalchemy`, `adbc_stage_table_sql` + `supports_upsert_adbc`) and structural overrides only where the portable form is invalid (`batch_commits_key_type`, `current_timestamp_default`). API connectors carry only the definition — no package files. Guide: `src/skills/connector-spec-db/spec-connector-package.md`.
...
The connector's `version` field is the connector release semver, bumped per the connector release table (patch/minor/major) by `connector-drift-classifier`; first release is `1.0.0`. This is distinct from the plugin's own package version in `src/.claude-plugin/plugin.json`, which is managed by **release-please** (Conventional Commits) — never bump it by hand:
...
Authored documents declare `$schema` with this host. Endpoint documents lock it to the `.ai` URL with a `const`; the connector contract matches it by pattern and tolerates any environment host (`schemas.analitiq.<tld>`) — always author the canonical `.ai` URL either way. Type maps are bare JSON arrays and carry no `$schema` at all; the validator derives their direction from the filename. The validator matches on this URL offline; it does not fetch it.

The `connector-schema-validator` sub-agent runs the published **`analitiq-validator`** package (which the agent self-installs at runtime), performing single-document contract-model validation (structure **and** every cross-field rule) plus the cross-file checks listed under **Agents** above. Validation is **offline and model-driven** — no schema fetch. Note what it does *not* do: inline function names are never checked, connector-side ref resolvability is never checked, and there is no TLS `ssl_mode` ↔ `ssl_ca_certificate` consistency check — those stay author-side discipline (see `src/agents/connector-schema-validator.md`). The validator checks JSON documents only — the database package files are enforced by registry CI. The console entry point is `analitiq-validate`; registry CI installs the same package to run the semantic checks outside the plugin runtime. The published schema (generated from the same contract models) is the single source of truth. The plugin's own drift guard lives under `tests/schema_drift/`.
...
- **The live schema is the contract — enforce it, don't restate it.** The validator checks each document against the contract models (`analitiq-contract-models`, the same models the published JSON Schemas are generated from) **offline** — no runtime schema fetch — so authoring and validation agree on one contract. Where the plugin must restate a schema-owned enum as decision logic (e.g. the DSN-binding `encoding` set at `$defs/DsnBinding/properties/encoding`), the drift-check CI below pins it to the pinned contract models.
- **Cite cross-field rules, don't restate them.** The contract ships its own registry of relational rules (`analitiq.contracts.shared.advisory.all_rules()`), each with a stable `ADV-*` id. `scripts/render_advisory.py` renders the plugin-relevant subset to `src/skills/connector-builder/references/advisory-rules.md`, and `tests/schema_drift/` fails when that file is stale. Prose cites a rule id instead of paraphrasing the rule — a generated, CI-pinned copy, not a second source.
...
Canonical types are Apache Arrow logical types in PascalCase (e.g. `Int32`, `Int64`, `Float64`, `Utf8`, `Boolean`, `Binary`, `Date32`, `Time64`, `Timestamp`, `Decimal128`, `List`, `Struct`, `Map`). The vocabulary is owned by the published `analitiq-contract-models` package (the Arrow type vocabulary in `analitiq.contracts`) and the live `canonical-types.json` schema. Authoring guidance: `src/skills/connector-spec-db/spec-type-maps.md`.

## Conventions

- JSON Schema Draft 2020-12 throughout.
- `connector_id` is the stable connector slug; `^[a-z0-9][a-z0-9_-]*$`; immutable. The same value names the on-disk `{connector_id}/` directory.
