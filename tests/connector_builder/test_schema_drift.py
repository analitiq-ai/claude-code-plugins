...
def _authored_lines(path: Path) -> list[tuple[int, str]]:
    """The lines of `path` a person wrote, numbered.

    Generated text is skipped — a whole file a renderer owns, and any
    `BEGIN GENERATED` block inside a hand-authored one. Both are projections of
    the contract by construction, so a pattern appearing there is the rendered
    form of a value this repo owns, not the hand copy this guard hunts. Marker
    matching only: which lines to read, never what they mean.
    """
    lines = path.read_text(encoding="utf-8").splitlines()
    if any("GENERATED by" in line for line in lines[:3]):
        return []
    out, generated = [], False
...
def _slug_literal_sites() -> list[tuple[str, int, str]]:
    """Every (relpath, lineno, literal) slug-charset occurrence a person wrote."""
    return [
        (path.relative_to(PLUGIN_ROOT).as_posix(), lineno, match.group(0))
        for path in sorted(PLUGIN_ROOT.rglob("*.md"))
        for lineno, line in _authored_lines(path)
