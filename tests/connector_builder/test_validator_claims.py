...
def test_unknown_block_id_is_reported_as_a_malformed_marker(
    ...
    A `claim:*` pair authored under the pipeline tree parses there — both
    generators read one grammar — and no renderer under that tree owns it, so
    it is rendered by nobody and regenerated by nobody while looking
    machine-maintained.
