...
def test_a_stamp_of_the_render_shape_passes_the_check_until_the_release(monkeypatch, tmp_path):
    """Only the release re-stamps, so between releases the committed stamp
    names the models the published tree came from, not the models on main."""
    lagging = tmp_path / "contracts-version.json"
    doc = render_schemas.build_contracts_version_doc()
    doc[render_schemas.CONTRACTS_VERSION_KEY] = "0.0.0"
    lagging.write_text(json.dumps(doc))
    monkeypatch.setattr(render_schemas, "CONTRACTS_VERSION_PATH", lagging)
    assert render_schemas.check_contracts_version(released=False)[0]
    assert not render_schemas.check_contracts_version(released=True)[0]
...
def test_versionless_pyproject_fails_loudly(monkeypatch, tmp_path):
    ...
    with pytest.raises(RuntimeError):
        render_schemas.contract_models_version()
    ok, _ = render_schemas.check_contracts_version(released=True)
    assert not ok
...
def test_stale_stamp_names_the_fix(monkeypatch, tmp_path):
    ...
    stale.write_text('{"analitiq-contract-models": "0.0.0"}\n')
    monkeypatch.setattr(render_schemas, "CONTRACTS_VERSION_PATH", stale)
    ok, msg = render_schemas.check_contracts_version(released=True)
    assert not ok and "`render_schemas.py release`" in msg
