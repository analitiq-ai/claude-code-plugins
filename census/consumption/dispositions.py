...
DISPOSITIONS: tuple[FieldDisposition, ...] = (
    ...
    FieldDisposition(
        ...
        "by the slot the binding sits in, and the field is the copy that "
        "goes. The model validators that read it — the slot agreement, "
    ...
    FieldDisposition(
        ...
        "the named landing site (RULE-ENDP-002, RULE-ENDP-055, RULE-ENDP-070 "
        "and RULE-ENDP-071 name the obligations on the map's shape; "
    ...
    FieldDisposition(
        ...
        "and the manifest claims the handle, so the object the locator names "
        "settles what the run reads — a locator naming a different object is "
        "a different handle, not an ignored value. RULE-STRM-003 names the "
        "obligation binding a supplied handle to its locator, RULE-DBEP-007 "
        "the ban on "
