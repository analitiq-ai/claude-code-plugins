...
- ``derivation_input`` — the contract computes another field of the same
  ...
  the manifest claims, and one a parse-time derivation on the model — or on
  a class it inherits from — actually writes: a derivation whose product
  nothing reads is a gap, not a derivation, and one no code performs is not
  a derivation at all. Which field is computed from which, and why the input
...
class FieldDisposition:
    ...
    claiming it, or when no parse-time derivation in the model's class
    hierarchy writes it. That the derivation reads THIS field, rather than
    some other, is the reader's, under
