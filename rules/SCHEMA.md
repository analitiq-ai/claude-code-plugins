...
| `artifact_kinds` | yes | The artifact kinds it binds, as a list from the vocabulary `ARTIFACT_KINDS` declares in `analitiq.contracts.shared.rule_record` — each member names the kind of document whose author the rule binds, plus `any` for the rules that bind every authored document. A list because a rule can grade more than one kind, and the generated reference is split by artifact kind: a scalar here would decide, silently, which of two authors never meets the rule. `any` may not appear beside a named kind — it already covers every document — and an entry may not repeat. It is not derived from the published resources and does not track them: `connector-package` names a repository layout rather than one authored document, and several published resources have no member because no rule has needed one. A member is added when a rule does. `artifact_kinds` decides which FILE of a plugin's generated reference set the rule lands in; who the rule is rendered *to* is `owners`. |
...
| `status` | yes | `draft` \| `active` \| `deprecated` \| `retired`. The lifecycle, and the reason no record carries a boolean — `active` is not the opposite of any one thing. A `draft` is written down but not yet in force; a `deprecated` rule still binds while authors are moved off it, so prose citing it still resolves; a `retired` record stays on disk to point prose citing the rule at the id that replaced it, which is why a retirement names a successor. An obligation withdrawn outright has no successor to name: its record is removed, and its id is claimed by `RETIRED_WITHOUT_A_RECORD` in `analitiq.contracts.shared.rule_record`. Either way the id is never reissued. |
| `superseded_by` | no | The id that replaced this one. Required when `status: retired`, which is why an obligation withdrawn with nothing replacing it takes the other retirement route described under `status` — the record is deleted and the id claimed in `RETIRED_WITHOUT_A_RECORD`. |
...
| `path` | Where the finding applies. A path that is empty or starts with `/` is a JSON Pointer into the document that was validated, in RFC 6901 §5 string form; the empty pointer is the whole document. Any other path is `<reference>#<pointer>`, about another document the check read: the reference is that document's path relative to the validated document's directory, percent-encoded as an RFC 3986 relative reference, and the pointer is the same §5 form, never percent-encoded. A consumer splits at the first `#` and percent-decodes only the reference. A package result has no single validated document, so a finding about one entry of the package is `<key>#<pointer>`, the key read from the package root and encoded as a reference is. The entry is one the package carries — a key the caller submitted, or a directory one of those keys implies, which is how a directory named like a document is reported to the author who built it. A finding no entry is the subject of is about the package as a whole and carries the empty path, which reads there as it does for a single document: the whole of what the call was handed. |
