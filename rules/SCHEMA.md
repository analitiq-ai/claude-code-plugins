...
| `pattern_symbol` | no | The dotted `module::NAME` holding the regex a `mechanism: pattern` rule is about, so the rendered reference can print the form the statement points at. Read from the record rather than off the field, because a field carries one pattern per rule that grades it and nothing in the shape says which is whose — printing them all under each rule states what the field must satisfy and never what the rule requires. Resolved by import like `validator`, so a renamed constant fails the build. Refused on a record whose `mechanism` is anything else. |
