...
| `path` | Where the finding applies. A path that is empty or starts with `/` is a JSON Pointer into the document that was validated, in RFC 6901 §5 string form; the empty pointer is the whole document. Any other path is `<reference>#<pointer>`, about another document the check read: the reference is that document's path relative to the validated document's directory, percent-encoded as an RFC 3986 relative reference, and the pointer is the same §5 form, never percent-encoded. A consumer splits at the first `#` and percent-decodes only the reference. A package result has no single validated document, so a finding about one of them is `<key>#<pointer>`, the key read from the package root and encoded as a reference is, and always a key the caller submitted. A finding no submitted document is the subject of is about the package as a whole and carries the empty path, which reads there as it does for a single document: the whole of what the call was handed. |
