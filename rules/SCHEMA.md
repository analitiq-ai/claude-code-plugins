...
| Something worth surfacing about how a check proceeded, with no rule to bind it — the type-map gap report specified by `analitiq.validator.document_set.resolve_type_map_gaps`, which names the probes no map rendered. That function raises `NotImplementedError`, so no `informational` finding is emitted. | `informational` | none |
