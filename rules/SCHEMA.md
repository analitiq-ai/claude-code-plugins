...
| `path` | Where the finding applies. A JSON Pointer (empty, or starting with `/`) points into the document that was validated; the empty pointer is the whole document. A finding about another document the check read beside it is `<reference>#<pointer>`: the reference is that document's path relative to the validated document's directory, percent-encoded as an RFC 3986 relative reference, so the first `#` always separates it from the pointer. A package result has no single validated document, so each of its findings is `<key>#<pointer>`, the key encoded as a reference is and read from the package root. |
