...
7. **Author connection-scoped type-map gap rules** per
   ...
   - Probe the distinct frozen `arrow_type` strings against the write maps (connection first if present, then connector); for
...
3. Resolve every distinct `arrow_type` through the write maps with
   `type_map_gaps.py` (connection map first if present,
