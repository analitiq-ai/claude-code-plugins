...
all hold: 14 of the 37 below have no validator, so nothing rejects
...
In this file: **16** shape · **8** coherence · **5** reference · **6** process · **2** choice.
...
| RULE-TMAP-005 | A `regex` read rule's native_type pattern MUST compile under the RE2 regex dialect, and a named capture in it MUST be spelled `(?<name>…)` under a name no other group carries. | `type-map` | error | validator | — |
...
| RULE-TMAP-018 | A connection-scoped type map MUST declare a rule only for a native_type or arrow_type its connector's own map leaves unresolved. | `type-map` | error | validator |
