...
| RULE-TMAP-005 | A `regex` read rule's native_type pattern MUST compile under the RE2 regex dialect and MUST NOT carry Python-only `(?P…)` syntax, so a named capture is spelled `(?<name>…)`. | `type-map` | error | validator | — |
...
| RULE-TMAP-009 | A write `regex` rule's arrow_type matcher MUST compile under the RE2 regex dialect and MUST NOT carry Python-only `(?P…)` syntax, so a named capture is spelled `(?<name>…)`; the native_type DDL it renders MUST carry only well-formed placeholders. | `type-map` | error | validator | — |
