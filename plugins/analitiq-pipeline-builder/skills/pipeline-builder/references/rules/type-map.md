...
| RULE-TMAP-005 | A `regex` read rule's native_type pattern MUST compile under the RE2 regex dialect, a named capture in it MUST be spelled `(?<name>…)`, and it MUST NOT repeat anything zero times. | `type-map` | error | validator | — |
...
| RULE-TMAP-009 | A write `regex` rule's arrow_type matcher MUST compile under the RE2 regex dialect with any named capture spelled `(?<name>…)` and no zero-count repetition, and the native_type DDL it renders MUST carry only well-formed placeholders. | `type-map` | error | validator | — |
