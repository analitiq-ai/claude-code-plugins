...
| RULE-STRM-021 | A validation rule's `value` MUST carry a payload of the shape its `type` reads, and that payload MUST constrain something: a length MUST be a non-negative integer, a pattern a non-empty regular expression, a range an object carrying a `min` or a `max` and no other bound and no empty interval, an admitted-value set a non-empty list. | `stream` | error | validator |
