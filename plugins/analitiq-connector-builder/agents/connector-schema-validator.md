...
# contaminate the Diagnostics JSON). Pin BOTH and keep them in lockstep: rc19's
# validator already pins analitiq-contract-models==1.0.0rc19 exactly, so this is
# defensive (explicit + reproducible, and safe if a future validator loosens it).
python3 -c "import sys; from importlib.metadata import version; sys.exit(0 if version('analitiq-validator') == '1.0.0rc19' and version('analitiq-contract-models') == '1.0.0rc19' else 1)" 2>/dev/null \
  || python3 -m pip install --quiet --disable-pip-version-check --pre "analitiq-validator==1.0.0rc19" "analitiq-contract-models==1.0.0rc19" 1>&2
