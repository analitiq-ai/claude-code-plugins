...
1. **Contract-model validation** — structure, plus the cross-field rules the
   ...
   A rejection here carries the `RULE-*` id of the rule it violates; a field
   constraint the model enforces on its own, with no rule behind it, carries
   none. Every rule an author must satisfy is catalogued by id in
...
`analitiq-contract-models` package — the same contract the validator enforces —
so it runs **offline** with no schema fetch. From the repo root:
`pip install -r requirements-dev.txt` then `pytest tests/connector_builder/`.
Without the package installed the guards skip.
...
pipeline-builder plugin, and how to develop against a local checkout.
