...
- **The pre-split `type-map.json`** — a bare rules array with no envelope to
  declare a direction, so nothing reads it at either scope. A connector ships a
  map declaring each direction its `kind` calls for (`RULE-PKG-030`).
