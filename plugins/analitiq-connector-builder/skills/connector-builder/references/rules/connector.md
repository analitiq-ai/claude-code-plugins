...
Grades: every rule this plugin owns that binds a **`connector`** document, plus the rules that bind every authored document. If you are authoring one, this file is the whole of what you must satisfy — no other rule file in this set applies to it.
...
| RULE-CTOR-023 | A connector document's `connector_id` MUST match the slug pattern `ConnectorBase.connector_id` declares. | `connector` | error | validator | `^[a-z0-9][a-z0-9_-]*$` |
