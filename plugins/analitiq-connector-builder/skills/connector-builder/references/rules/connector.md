...
| RULE-CTOR-023 | A connector document's `connector_id` MUST match the slug pattern `ConnectorBase.connector_id` declares. | `connector` | error | validator | `connector_id` matches: `^[a-z0-9][a-z0-9_-]*$` |
