...
| RULE-PKG-030 | A connector whose `kind` resolves types MUST ship a sibling `type-map.json` carrying a rule list for each direction that kind renders and none for a direction it does not; no connector ships any other `type-map-*.json` beside it. | `connector-package` | error | validator |
