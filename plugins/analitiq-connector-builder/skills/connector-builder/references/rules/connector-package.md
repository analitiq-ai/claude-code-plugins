...
| RULE-PKG-030 | A connector MUST ship the sibling type-map documents its `kind` calls for as `type-map-read.json` and `type-map-write.json`, and MUST NOT ship one for a direction its kind has no use for. | `connector-package` | error | validator |
