...
| RULE-PKG-023 | A connector's write direction MUST live in its write type-map document: the package ships no Python type-rendering table, and a `render_column_type` override exists only for logic the map's rules cannot express, delegating every other type back to the map. | `connector-package` | warning | — | — |
...
| RULE-PKG-030 | A connector MUST ship a sibling type-map document declaring each direction its `kind` calls for, at most one document per direction, and MUST NOT ship one declaring a direction its kind has no use for. | `connector-package` | error | validator |
