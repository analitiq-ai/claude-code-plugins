...
| RULE-PKG-030 | A connector whose `kind` resolves types MUST ship a sibling `type-map.json` carrying a rule list for each direction its `kind` calls for and none for a direction it does not. | `connector-package` | error | validator |
