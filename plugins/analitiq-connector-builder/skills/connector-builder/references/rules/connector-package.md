...
| RULE-PKG-023 | A connector's write direction MUST live in the `write` section of its type map: the package ships no Python type-rendering table, and a `render_column_type` override exists only for logic the map's rules cannot express, delegating every other type back to the map. | `connector-package` | warning | — | — |
...
| RULE-PKG-030 | A connector whose `kind` resolves types MUST ship a sibling `type-map.json` carrying a rule list for each direction its `kind` calls for and none for a direction it does not; no connector ships any other `type-map-*.json` beside it. | `connector-package` | error | validator |
