...
| RULE-ENDP-066 | A landing site that spells the comparison into the value MUST carry the filter's value, through a placeholder whose leading token is a known resolution scope. | `api-endpoint` | error | validator |
| RULE-ENDP-067 | An operator MUST NOT name as its landing site a parameter declared as controlled by pagination or replication. | `api-endpoint` | error | validator |
