...
all hold: 19 of the 81 below have no validator, so nothing rejects
...
In this file: **34** structural · **31** advisory · **8** referential · **4** procedural · **4** judgment.
...
| RULE-ENDP-002 | A parameter the runtime controls MUST NOT be the landing site of a filter binding. | `api-endpoint` | error | validator |
...
| RULE-ENDP-066 | Two entries in a read operation's filter map MUST NOT send one parameter the same value. | `api-endpoint` | error | validator |
| RULE-ENDP-067 | A filter binding that renders its own value MUST interpolate the filter's value, and MUST read nothing else from the stream scope. | `api-endpoint` | error | validator |
