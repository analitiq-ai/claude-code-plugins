...
| RULE-ENDP-036 | An endpoint document's `endpoint_id` MUST match the slug pattern `_EndpointBase.endpoint_id` declares. | `api-endpoint` `database-endpoint` | error | validator | `endpoint_id` matches: `^[a-z0-9][a-z0-9_-]*$` |
...
| RULE-ENDP-060 | A brace in a request's `path` MUST delimit a `{name}` placeholder, and every such name MUST match the contract's placeholder-name pattern, regardless of how the provider spells the value that placeholder carries. | `api-endpoint` | error | validator | `path` matches: `^(?:[^{}]\|\{[a-z][a-z0-9_]*\})*$` · refuses: `\$\{`, `\{([a-z][a-z0-9_]*)\}[\s\S]*\{\1\}` |
| RULE-ENDP-061 | A request's `path` MUST NOT carry a `${...}` template expression; the only substitution into a path is the `{name}` placeholders `path_params` binds. | `api-endpoint` | error | validator | `path` matches: `^(?:[^{}]\|\{[a-z][a-z0-9_]*\})*$` · refuses: `\$\{`, `\{([a-z][a-z0-9_]*)\}[\s\S]*\{\1\}` |
