...
| RULE-ENDP-074 | Every replication cursor field MUST resolve to a record-shape node whose declaration names exactly one JSON type besides `null` — in its own `type`, or across every branch of an `anyOf`/`oneOf` — and that type MUST be one that can hold a cursor: a string moment or an integer. | `api-endpoint` | error | validator |
