...
all hold: 19 of the 79 below have no validator, so nothing rejects
...
In this file: **34** structural · **29** advisory · **8** referential · **4** procedural · **4** judgment.
...
| RULE-ENDP-002 | An operator MUST NOT name as its landing site a parameter declared as controlled by pagination or replication. | `api-endpoint` | error | validator |
...
| RULE-ENDP-066 | A landing site that renders the comparison into the value MUST interpolate the filter's value, through a placeholder whose leading token is a known resolution scope. | `api-endpoint` | error | validator |
