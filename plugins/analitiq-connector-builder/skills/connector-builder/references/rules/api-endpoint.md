...
| RULE-ENDP-066 | A param declared `required` MUST declare a source its operation can supply the value from: its own `default`, or — on a read — the `operators` that make it stream-filterable or a pagination or replication block that names it. | `api-endpoint` | error | validator |
...
| RULE-ENDP-067 | A param MUST be declared `required` only where its operation is wrong without the value: `required` is the contract's demand that such an operation fail rather than send the request with that param's slot dropped. | `api-endpoint` | error | — |
