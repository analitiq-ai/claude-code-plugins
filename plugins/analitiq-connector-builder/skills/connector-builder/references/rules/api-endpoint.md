...
| RULE-ENDP-063 | A node of an endpoint's response or write-input schema that records wire samples under `examples` and declares — on the node, or one fold away in a `$ref` target or `allOf` branch — a canonical Arrow type whose family carries a timezone position MUST declare one whose zone-awareness those samples bear out, and each such sample written as a date-time MUST name a moment that exists. A node reached through a position whose subschema does not describe the instance is exempt — `not`, which says what the instance must not be; `if`, which selects a branch rather than constraining; and `propertyNames`, which constrains the KEYS. A sample recorded at one of those is not a value the declaration beside it claims to describe, so agreement between them says nothing either way. Recording a sample never obliges a node to declare a type it has no evidence for. | `api-endpoint` | error | validator |
| RULE-ENDP-064 | Every entry a node of an endpoint's response or write-input schema records under `examples` MUST satisfy that node's own declarations, unless the node is reached through a position whose subschema does not describe the instance — `not`, which says what the instance must not be; `if`, which selects a branch rather than constraining; and `propertyNames`, which constrains the KEYS. A sample recorded at one of those is not a value the declaration beside it claims to describe. | `api-endpoint` | error | validator |
