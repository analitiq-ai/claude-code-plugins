...
| RULE-ENDP-063 | A node of an endpoint's response or write-input schema that records wire samples under `examples` and declares a canonical Arrow type whose family carries a timezone position MUST declare one whose zone-awareness those samples bear out, and each such sample written as a date-time MUST name a moment that exists. A node reached through a position that describes what an instance must NOT be is exempt: the sample recorded there is a counter-example. Recording a sample never obliges a node to declare a type it has no evidence for. | `api-endpoint` | error | validator |
| RULE-ENDP-064 | Every entry a node of an endpoint's response or write-input schema records under `examples` MUST satisfy that node's own declarations, unless the node is reached through a position that describes what an instance must NOT be — where the sample is a counter-example, and satisfying the declaration beside it would be the defect. | `api-endpoint` | error | validator |
