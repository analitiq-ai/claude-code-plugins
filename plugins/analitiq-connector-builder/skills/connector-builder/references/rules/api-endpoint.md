...
| RULE-ENDP-022 | An expression an API endpoint operation declares MUST be the shape its own form models: exactly one expression key, no sibling beyond the argument fields that key itself declares and extension keys, and a payload of the modelled type — a string `ref`, a string `template` that closes every `${` it opens, and a `function` naming a non-empty string with only the arguments the function expression declares. | `api-endpoint` | error | validator | — |
...
| RULE-ENDP-044 | A keyset pagination block MUST omit `initial` when there is no first-page key, and MUST NOT spell that absence as an explicit null. | `api-endpoint` | warning | validator | — |
...
| RULE-ENDP-074 | Every replication cursor field MUST be a whole key in the record shape's own `properties`, as the engine walks to it — `response.records` followed through each node's own `properties` to the array's own `items`, following no `$ref` or `allOf` — on a node whose own `type` names exactly one JSON type besides `null`, and that type MUST be one that can hold a cursor: a string moment or an integer. | `api-endpoint` | error | validator |
...
| RULE-ENDP-078 | A replication cursor field typed `integer` MUST NOT declare a calendar `format` (`date-time`, `date`). An epoch `format` (`epoch_seconds`, `epoch_milliseconds`) makes it a moment; any other `format`, such as a provider width token like `int64`, or none makes it an id. A cursor mapping over an id MUST declare no `format` and MUST NOT take the window form. | `api-endpoint` | error | validator |
...
| RULE-ENDP-067 | A param MUST be declared `required` if and only if its operation is wrong without the value: `required` is the contract's demand that such an operation fail rather than send the request with that param's slot dropped, so leaving the flag off a param the operation cannot be correct without is the same defect as setting it on one whose absence the operation tolerates. | `api-endpoint` | error | — |
