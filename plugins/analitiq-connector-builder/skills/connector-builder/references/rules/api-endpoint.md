...
| RULE-ENDP-074 | Every replication cursor field MUST be declared as a plain key under the record shape's own `properties`, on a node whose own `type` names exactly one JSON type besides `null`, and that type MUST be one that can hold a cursor: a string moment or an integer. | `api-endpoint` | error | validator |
