...
| RULE-CTOR-022 | A post-auth output MUST state how its value is produced and where it is durably stored using only the vocabulary `PostAuthOutput` declares for each; which pairings of those values are legal is RULE-CTOR-002. | `connector` | error | `mode`: `user_selection`, `auto_discovery` · `storage`: `connection.selections`, `connection.discovered`, `secrets` |
