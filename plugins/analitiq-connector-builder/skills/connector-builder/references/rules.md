...
|---|---|---|---|---|
| RULE-CTOR-016 | An ADBC transport's `driver` MUST name a member of the closed vocabulary the contract declares for it; a driver the vocabulary does not name cannot be declared at all. | `connector` | error | `postgresql`, `snowflake`, `bigquery` |
| RULE-CTOR-017 | A bulk-load family key MUST name a mechanism the contract admits under that family — the families' vocabularies differ, so a mechanism is declarable only under the family whose field carries it — and a mechanism outside them is not declarable at all. | `connector` | error | `copy_from`, `load_data_local_infile`, `load_job`, `adbc_ingest` |
| RULE-CTOR-018 | Every binding in a `url_template` DSN MUST declare an `encoding` from the closed vocabulary the contract carries, selected for the URL position the value is substituted into. | `connector` | error | `raw`, `host`, `url_userinfo`, `url_path_segment`, `url_query_key`, `url_query_value` |
| RULE-CTOR-019 | A `resource_discovery.produces` entry MUST name an artifact kind the contract's vocabulary admits, and discovery MUST write nothing outside it. | `connector` | error | `connection.endpoints`, `connection.type_map` |
| RULE-CTOR-020 | Each discovery action a connector configures on `ResourceDiscoveryTriggers` MUST name its trigger from the closed vocabulary that model's matching field declares. | `connector` | error | `on_activation`, `on_connection_selected`, `on_resource_selected`, `on_demand`, `scheduled` |
| RULE-CTOR-021 | A connection-contract input MUST state how it is provisioned, when in the connection lifecycle it is provisioned, where its resolved value is durably stored and what value type it carries using only the vocabulary `ConnectionContractInput` declares for each. | `connector` | error | `connection.parameters`, `secrets`, `string`, `integer`, `number`, `boolean`, `array`, `object` |
| RULE-CTOR-022 | A post-auth output MUST state how its value is produced and where it is durably stored using only the vocabulary `PostAuthOutput` declares for each; which pairings of those values are legal is RULE-CTOR-002. | `connector` | error | `connection.selections`, `connection.discovered`, `secrets` |
...
| RULE-ENDP-038 | An endpoint's `replication.supported_methods` MUST name only methods the vocabulary `Replication.supported_methods` declares, and the block MUST NOT carry a separate default-method key. | `api-endpoint` | error | `full_refresh`, `incremental` |
| RULE-ENDP-039 | A write operation's `idempotency` MUST declare only where the provider's key is placed, from the placement vocabulary `Idempotency.location` carries, and MUST NOT carry anything that produces the key's value. | `api-endpoint` | error | `header`, `body` |
