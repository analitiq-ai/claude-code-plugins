...
<!-- PROBE: connector-schema-optional, endpoint-schema-host-locked, type-map-schema-required, type-map-direction-from-document, type-map-direction-not-schema-url, type-map-coverage-counts-declarations, type-map-duplicate-direction-rejected, type-map-duplicate-direction-covers-nothing -->
...
| Type maps | Required, locked to the direction's `.ai` URL by a `Literal` (`type-map-read` / `type-map-write`). | Yes — required, and a mismatched or missing `$schema` is rejected. A map is graded as the direction its own `direction` field names — not its `$schema`, and not its filename — whether it is validated on its own or inside a connector package. Which directions a package covers is counted from what its maps declare, so two maps declaring one direction is rejected and neither is the map for it (`RULE-PKG-030`); the filename only decides which siblings are type-map documents at all. |
