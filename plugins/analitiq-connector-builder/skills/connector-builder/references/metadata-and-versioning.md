...
<!-- PROBE: connector-schema-optional, endpoint-schema-host-locked, type-map-schema-required, type-map-rule-graded-by-section -->
...
| Type map | Required, locked to the `.ai` `type-map` URL by a `Literal`. | Yes — required, and a mismatched or missing `$schema` is rejected. The `$schema` names no direction: each rule is graded as the section it sits under, `read` or `write`. |
