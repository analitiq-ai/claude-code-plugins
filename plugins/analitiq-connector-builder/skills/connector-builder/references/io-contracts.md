    ...
    "fields": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "sample_value": { "description": "One value for this field copied verbatim out of a real payload, recorded in the JSON kind the provider sends it as — the string `\"0\"` and the boolean `false` are different evidence. Never composed to match the declared type, and a placeholder a documentation renderer synthesized is not a wire value. Carry it for any field a payload shows one for. A temporal's zone-awareness is decided on this value rather than guessed (`RULE-SHRD-002`); a temporal a payload shows no sample for carries no `sample_value` and no `native_type`/`arrow_type` pair, reported as a gap in `notes` rather than a zone assumed in its place. The read operation's record payload is evidence for `read`; a write mode's input payload is evidence for `write`; a body a read sends to filter its search is evidence for neither." },
