    ...
    "fields": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "write_modes": { "type": "array", "minItems": 1, "items": { "type": "string" }, "description": "Which write modes accept this field, when the provider does not accept the same set for all of them (an `insert` that takes fields an `upsert` refuses). Omitted means every mode the resource declares. Only meaningful on an entry whose `directions` include `write`." },
