    ...
    "fields": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "arrow_type": { "type": "string", "description": "Canonical Arrow type (PascalCase). For temporals, read the zone off `sample_value` (`RULE-SHRD-002`): a zoneless wire value → bare `Timestamp(<unit>)`; a value carrying an offset/Z → `Timestamp(<unit>, UTC)`. Where the sample and the documented type disagree, the sample decides — it is what the provider sends — and `notes` records the disagreement." },
