...
```json
{
  ...
  "properties": {
    ...
    "request_params": { "type": "object", "additionalProperties": { "type": "object", "required": ["in", "type"], "properties": { "in": { "type": "string", "description": "Where the provider takes it, from the placement vocabulary `RULE-ENDP-050` prints. A search that takes its filters in the POST body takes `body`." }, "type": { "type": "string", "description": "The request-input type, from the vocabulary `RULE-ENDP-050` prints — what is sent UP, which is not the field's `native_type`/`arrow_type`: a timestamp filtered as an ISO string is `string` however the field comes back." }, "style": { "type": "string", "description": "Documented serialization for a container-typed query parameter, which `RULE-ENDP-003` requires alongside `explode` — a provider accepts one spelling of a repeated key and the creator may not guess which." }, "explode": { "type": "boolean", "description": "Its companion, on the same rule." }, "enum": { "type": "array", "items": { "type": "string" }, "description": "Closed value domain, when the parameter is enumerated in the docs." }, "format": { "type": "string" } } }, "description": "The provider's documented request parameters for this resource's read, keyed by the provider's own name verbatim — everything a param declaration needs, recorded once here rather than repeated at each place that names one. `fields[].filterable` names entries of this map; a parameter serving pagination or replication is not recorded here, since those are declared from their own facts."},
    ...
    "fields": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "filterable": { "type": "object", "additionalProperties": { "type": "string" }, "description": "Which comparisons the provider accepts on this field when reading it, and which request parameter carries each: one entry per operator, keyed by the operator from the vocabulary `RULE-ENDP-055` prints, valued by a key of this resource's `request_params`. A provider that accepts two comparisons through one parameter offers only one of them — a parameter carries one value, so the second has nowhere to go — and the researcher records the one the docs make useful. Omitted means the resource documents no filtering on this field, which is different from documenting that it refuses it: `notes` says which. Only meaningful on an entry whose `directions` include `read`."},
