      ...
      "properties": {
        ...
        "resources": {
          ...
          "items": {
            ...
            "required": ["key"],
            "properties": {
              "key": { "type": "string", "description": "Stable resource id — becomes the endpoint_id, which names the endpoint document's file (`RULE-PKG-031`) (pattern ^[a-z0-9][a-z0-9_-]*$). DERIVE it, do not free-pick it (`RULE-ENDP-046`): flatten the resource's full locator, lowercased, with `__` between path levels so the full path — not just the leaf — distinguishes the id. API: every path segment in order, with path-params dropped (they are operation-level) — `/v1/blah/something/customer` → `v1__blah__something__customer`, `/v2/blah/something/customer` → `v2__blah__something__customer`. Deterministic: the same locator yields the same key on every re-author. (Database resources are not authored here — `RULE-DBEP-006`; the ids runtime discovery derives are opaque handles over a verbatim `database_object` — `RULE-DBEP-007`.)" },
              "label": { "type": "string" },
              "method": { "type": "string" },
              "path": { "type": "string" },
    ...
    "fields": {
      ...
      "items": {
        ...
        "properties": {
          ...
          "native_type": { "type": "string", "description": "Provider's documented/observed wire-type token (e.g. `string`, `integer`, `date-time`). Omitted, with `arrow_type`, for a field whose wire type the provider documents nowhere, and for a date-time field no payload shows a sample for — the creator then leaves that node untyped rather than guessing (`RULE-ENDP-006`, `RULE-ENDP-062`), and `notes` says which fields those were." },
          "arrow_type": { "type": "string", "description": "Canonical Arrow type (PascalCase). For date-time fields, chosen from the SAMPLE value's zone-awareness (`RULE-SHRD-002`): a zoneless wire value → bare `Timestamp(<unit>)`; a value carrying an offset/Z → `Timestamp(<unit>, UTC)`. A date-only field is `Date32` on its documented `format`." },
          ...
          "sample_value": { "description": "One value for this field copied verbatim out of a real payload, recorded in the JSON kind the provider sends it as — the string `\"0\"` and the boolean `false` are different evidence. Never composed to match the declared type, and a placeholder a documentation renderer synthesized is not a wire value. Carry it for any field a payload shows one for. A date-time field's zone-awareness is decided on this value rather than guessed (`RULE-SHRD-002`), and only a date-time string carries that evidence: where a payload shows no sample, or shows one that is `null` or otherwise not a date-time string, the field carries no `native_type`/`arrow_type` pair and the gap goes in `notes` rather than a zone being assumed in its place. A date-only field carries no zone question, so its pair stands on the documented `format` alone. The read operation's record payload is evidence for `read`; a write mode's input payload is evidence for `write`; a body a read sends to filter its search is evidence for neither." },
          "tz_aware": { "type": "boolean", "description": "For date-time fields, set only when a sample date-time string was observed: true iff that value carries a zone/offset. Omitted when none was — the same case that leaves the field without a type pair — so an absent flag reads as no evidence rather than as a zoneless value." }
        }
      }
    },
    "notes": { "type": "string" }
      ...
      "anyOf": [
        {
          ...
          "description": "Shape of the connector's type map: a `{$schema, read, write}` document. Each `read` rule has `native_type` as the matcher (regex patterns authored UPPERCASE) and `arrow_type` as the rendered Arrow type (may carry ${name} substitutions backed by named captures in `native_type`); each `write` rule inverts that — `arrow_type` is the matcher (regex with named captures for parameterized types) and `native_type` the rendered DDL. Which sections each kind must carry, and must not: `RULE-PKG-030`. Arrow-vocabulary coverage of `write`: `RULE-TMAP-017`; when a family may be left unrendered: `RULE-TMAP-019`. Written by the orchestrator as the connector's type map."
        },
