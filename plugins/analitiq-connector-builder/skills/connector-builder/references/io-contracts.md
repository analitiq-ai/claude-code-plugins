...
```json
{
  ...
  "properties": {
    ...
    "request_body": { "type": "object", "additionalProperties": true, "description": "For a read whose method takes a body, that body as `operations.read.request.body` would declare it: the shape the provider documents, with a `{\"from_param\": \"<name>\"}` binding at each position a per-run value belongs and literal values everywhere else. Recorded as the thing to author rather than as a description of it, because a body param's POSITION is not part of its declaration — `in: \"body\"` names a body and not a place in one, and the binding may sit at any depth. A creator given only the parameters would bind at the top level and build a request the provider ignores. Omitted for a read with no body."},
